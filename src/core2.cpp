...
void loop1(){
  ...
  if(millis() - lastRefresh > 50){ // yeah this fixes issue below. theres 2 hours ill never get back
...
for(int i = 0; i < NUM_VOICES; i++){
  ...
  if(noteNumber == 1 || noteNumber == 6) { // c#/f#
    display.drawBitmap(9 + (16 * i), 44, sharp, 8, 8, 1);
    noteNumber--;

  } else if(noteNumber == 3 || noteNumber == 8 || noteNumber == 10){ // Db/Eb/Bb
